...
void LitPassGeom(triangle Varyings input[3], inout TriangleStream<Varyings> outStream)
{
    ...
    float randomHeight = rand(input[0].positionWSAndFogFactor.xyz * 2.326764f);
    ...
    Varyings o2 = input[0];
    float3 o2Pos = (basePos + rotatedTangent*_Base + rotatedNormal*clamp(_Height*randomHeight,_MinHeight,_MaxHeight));
    ...
    o3.uv = TRANSFORM_TEX(float2(0,1),_BaseMap);
    o2.uv = TRANSFORM_TEX(float2(1,1),_BaseMap);
    o1.uv = TRANSFORM_TEX(float2(1,0),_BaseMap);
